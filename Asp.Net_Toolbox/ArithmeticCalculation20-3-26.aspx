<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ArithmeticCalculation20-3-26.aspx.cs" Inherits="WebApplication2.ArithmeticCalculation" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title></title>
</head>
<body>
    <form id="form1" runat="server">
        <div>
        </div>
        <br /><br />
        <asp:Label ID="Label1" runat="server" Text="number1"></asp:Label>
        <asp:TextBox ID="TextBox1" runat="server"></asp:TextBox>
        <p>
            <asp:Label ID="Label2" runat="server" Text="number2"></asp:Label>
            <asp:TextBox ID="TextBox2" runat="server"></asp:TextBox>
        </p>
        <p>
            <asp:Label ID="Label3" runat="server" Text="number3"></asp:Label>
            <asp:TextBox ID="TextBox3" runat="server"></asp:TextBox>
        </p>
        <asp:Button ID="Button1" runat="server" OnClick="Button1_Click" Text="ADD" />
        <asp:Button ID="Button2" runat="server" OnClick="Button2_Click" Text="SUB" />
        <asp:Button ID="Button3" runat="server" OnClick="Button3_Click" Text="MULT" />
        <asp:Button ID="Button4" runat="server" OnClick="Button4_Click" Text="DIV" />
    </form>
</body>
</html>
